...
/////////////////////////////////////////////////////////////////////////////
BOOL CQuantizer::ProcessImage(Image &hImage)
{
	BYTE r,	g, b, a;
	int	i, j;

	int effwdt=((((hImage.depth * hImage.width)+ 31)/32)*4);
	int	nPad=effwdt-(((hImage.depth * hImage.width)+7)/8);
	BYTE* pbBits=hImage.image;
	...
	case 4:	// 4-bit DIB
	case 8:	// 8-bit DIB
		for	(i=0; i<hImage.height; i++)
		{
			for	(j=0; j<hImage.width; j++)
			{
				BYTE idx=GetPixelIndex(j,i,hImage.depth,effwdt,pbBits);
				BYTE *pal=(BYTE*)hImage.palette;
				long ldx=idx*sizeof(RGBQUAD);
				b = pal[ldx];
				g = pal[ldx+1];
				r = pal[ldx+2];
