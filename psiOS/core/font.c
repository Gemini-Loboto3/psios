...
FONT_GLYPH *FontGetGlyph(const FONT_DATA *fnt, const u32 index)
{
	...
	for(i=0; i<size; i++)
	{
		...
		if(index>=fnt->ptr[i].min && index<=fnt->ptr[i].max)
		{
			...
			{
			case FT_REFERENCE:
				sptr=(u32*)(fnt->data+fnt->ptr[i].ptr);
			...
			case FT_INDEX:
				ref=(u8*)&fnt->data[fnt->ptr[i].ptr];
				// pointers
				idxp=(FONT_INDEX*)ref;
				// index lookup table
				idx=(u16*)&ref[4+idxp->count*4];
...
*/
static __inline void FontOpenReader(FONT_BITREAD *b, u16* data)
{
	b->data=(u8*)data;
	b->bit=0;
	b->byte=0;
...
*/
static u32 FontReadBits(FONT_BITREAD *b, int nb)
{
	u32 val=0;
	int i;
